...
#define STAGE2	 			"/dev_flash/sys/stage2"
#define STAGE2_DEVBLIND		"/dev_blind/sys/stage2"

#if defined (FIRMWARE_CEX)
	#define STAGE2_FILE		STAGE2 ".cex"
#elif defined (FIRMWARE_DEX)
	#define STAGE2_FILE		STAGE2 ".dex"
#endif
...
void main(void)
{
	...
	if(stage2)
	{
		...
		// Disabling both stage2 to avoid issues, will be enabled in stage2
		cellFsUtilMount_h("CELL_FS_IOS:BUILTIN_FLSH1", "CELL_FS_FAT", "/dev_blind", 0, 0, 0, 0, 0);
		cellFsRename(STAGE2_DEVBLIND ".cex", STAGE2_DEVBLIND ".cex.bak");
		cellFsRename(STAGE2_DEVBLIND ".dex", STAGE2_DEVBLIND ".dex.bak");	
