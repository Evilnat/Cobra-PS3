...
#define STAGE2_BLIND		"/dev_blind/sys/stage2"
	...
	#define CB_LOCATION 	STAGE2_BLIND ".cex"
#elif defined (FIRMWARE_DEX)
	#define CB_LOCATION 	STAGE2_BLIND ".dex"
#endif
	...
	switch (function)
	{
		case SYSCALL8_OPCODE_PS3MAPI:
			switch ((int)param1)
			{
				...
				case PS3MAPI_OPCODE_TROPHY:
					hidden_trophy_mode = (uint8_t)param2;
					return SUCCEEDED;
				break;	
...
static void cobra_loaded()
{
	// Cobra loaded successfully
	// By bguerville and aldostools
	cellFsUtilMount_h("CELL_FS_IOS:BUILTIN_FLSH1", "CELL_FS_FAT", "/dev_blind", 0, 0, 0, 0, 0);
