	...
	while (remaining > 0)
	{
		...
		if (file != UNDEFINED)
		{
			if (discfd == UNDEFINED || file != activefile)
			{
				...
				{
					CellFsStat stat;
					if(cellFsStat(discfile->files[file], &stat) == SUCCEEDED)
					{
...
static void build_netemu_params(uint8_t *ps2_soft, uint8_t *ps2_net)
{
	...
	// Now ps2bootparam
	if (cellFsOpen("/dev_hdd0/tmp/game/ps2bootparam.dat", CELL_FS_O_WRONLY | CELL_FS_O_CREAT | CELL_FS_O_TRUNC, &fd, 0666, NULL, 0) != SUCCEEDED)
	{
