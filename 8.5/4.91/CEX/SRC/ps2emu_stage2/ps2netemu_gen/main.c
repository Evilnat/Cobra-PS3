...
#define JUMP_ADDR 					0x27E300 // Imported by Evilnat thanks to @3141card's ps2_netemu research
#define TEMPERATURE_PAYLOAD_ADDR 	0x1FEEF8 // Imported by Evilnat thanks to @3141card's ps2_netemu research
#define TEMPERATURE_MAIN_ADDR		0x1FF038 // Imported by Evilnat thanks to @3141card's ps2_netemu research
...
static uint8_t hvcall_perm_patch1[0x10] = 
	{
		0xE8, 0x01, 0x00, 0x80, 0x38, 0x21, 0x00, 0x70, 
...
static int patch_temperature_payload(FILE *file)
{
	uint32_t blr = swap32(BLR);
	uint32_t li = swap32(LI(10, 2));
	uint32_t lir3 = swap32(LI(3, 0));	
	uint32_t nop = swap32(NOP);	

	fseek(file, 0, SEEK_END);
	uint32_t elf_size = ftell(file);
	fseek(file, 0, SEEK_SET);

	uint8_t *elf_buf = malloc(elf_size);
	fread(elf_buf, 1, elf_size, file);

	// Patches 
	// Offsets should be the same in later firmwares, but may change
	memcpy(elf_buf + 0x1FEEF0, &lir3, 4); // Patch sub_1EEEF0 to return 0
	memcpy(elf_buf + 0x1FEEF0 + 4, &blr, 4); // Patch sub_1EEEF0 to return 0	 
	memcpy(elf_buf + 0x1FF174, &nop, 4); // NOP
	memcpy(elf_buf + 0x20000, &blr, 4); // Disable check_text_for_illegal_instructions()
	memcpy(elf_buf + 0x21E20, &nop, 4);	// Disable check_text_for_illegal_instructions()
	memcpy(elf_buf + 0x204D4, &blr, 4); // Patch invalidate_gpu_hvcalls() syscall
	memcpy(elf_buf + 0x20578, &li, 4); // Patch set_hvcall_perm_for_initialize()
	memcpy(elf_buf + 0x2085C, &li, 4);	
	memcpy(elf_buf + 0x20598, &hvcall_perm_patch1, 0x10); // Patch set_hvcall_perm_for_initialize()
	memcpy(elf_buf + 0x20878, &hvcall_perm_patch1, 0x10); // Patch set_hvcall_perm_for_initialize()
	
	// Jump to load custom payload
	uint32_t jump = MAKE_JUMP_VALUE(JUMP_ADDR, TEMPERATURE_MAIN_ADDR);
	jump = swap32(jump + 1); 	
	memcpy(elf_buf + JUMP_ADDR, &jump, 4);

	fseek(file, 0, SEEK_SET);
	fwrite(elf_buf, 1, elf_size, file);	

	FILE *temp_payload = fopen("temperature_payload.bin", "rb");

	if(!temp_payload)
	{
		printf("Cannot open temperature_payload.bin\n");
		return -1;
	}
	
	fseek(temp_payload, 0, SEEK_END);
	uint32_t temp_payload_size = ftell(temp_payload);
	fseek(temp_payload, 0, SEEK_SET);

	uint8_t *temp_payload_buf = malloc(temp_payload_size);
	...
	fread(temp_payload_buf, 1, temp_payload_size, temp_payload);
	fseek(file, TEMPERATURE_PAYLOAD_ADDR, SEEK_SET);
	fwrite(temp_payload_buf, 1, temp_payload_size, file);
...
int main(int argc, char *argv[])
{
	...
	if(patch_temperature_payload(elf))
	{
		printf("Cannot patch with temperature payload\n");
		return -5;
	}
